...
La implementación de esta funcionalidad será como la anterior, ya que también será una clase singleton llamada 
\begin{otherlanguage} {english}``\textit{\textbf{Logger}}''\end{otherlanguage}, para tener control sobre la única
instancia existente y que sea más eficiente, ya que será más fácil de mantener una sola instancia. Esta se encargará 
de inicializar el objeto responsable de los registros, indicando el nivel mínimo de los registros y la ruta del 
archivo que también guardará dichos registros. Esto será cargado gracias a las propiedades del sistema.