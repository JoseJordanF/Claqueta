...
Este hito es muy breve, ya que se pretende obtener un sistema de configuración que permita la futura configuración
de otras herramientas, de manera que no sea necesario cambiar gran parte del código fuente. Permitiendo que solo 
se cambien los ficheros de configuración pertinentes; como primer paso hay que definir las fuentes de las que 
proviene dicha configuración, dado que la aplicación se desarrolla en Kotlin se ha decidido recurrir a las fuentes 
más populares en aplicaciones de este lenguaje \cite{popularConfig} como JSON, ficheros Java Properties, YAML e 
incluso ficheros INI. Por lo tanto, al finalizar este hito se debería obtener un PMV, siendo este el proyecto 
integrado con un sistema de configuración que permita adaptar la aplicación a la fuente de la que provenga la 
configuración sin cambiar el código fuente. Brindando de esta manera privacidad del contenido sensible guardado en 
los ficheros de configuración y flexibilidad a la hora de permitir diferentes fuentes de las que provengan estos. 

Este sistema de configuración facilita el mantenimiento y la gestión de esta configuración. Por último, permite 
escalar la aplicación de manera más eficiente al proporcionar una forma de ajustar la configuración para optimizar 
el rendimiento y la escalabilidad según sea necesario.

De modo que se debe encontrar la herramienta que permita controlar la configuración de la aplicación. 
Para Kotlin existen mucha variedad de bibliotecas, pero se busca una que permita leer de las distintas fuentes
vistas antes, o al menos de la mayoría. Se busca principalmente en el repositorio oficial de Maven 
\footnote{\url{https://mvnrepository.com/}}, ya que brinda una gran cantidad de opciones, además de más 
información relacionada con la popularidad de la biblioteca o las vulnerabilidades que estas tienen. Las 
bibliotecas encontradas serían las siguientes, Apache Commons Configuration, Config de Lightbend, Config Magic, 
Config de NetworkNT, están listadas en orden de popularidad, la mayoría tiene soporte para leer de distintas 
fuentes como JSON o YAML, pero pocas pueden leer de más de una de estas fuentes vistas; sin embargo, Config de 
Lightbend y  Config de NetworkNT poseen soporte nativo para JSON, ficheros Java Properties y JSON, YAML 
respectivamente. Por ende cualquiera de estas dos bibliotecas puede ser escogida, pero Config de NetworkNT es una 
biblioteca menos popular respecto a Config de Lightbend y por esto es posible que haya menos documentación acerca 
del soporte para algunas fuentes. Además, Config de Lightbend permite leer de otras fuentes populares como HOCON 
(\begin{otherlanguage} {english}``\textit{\textbf{Human-Optimized Config Object Notation}}''\end{otherlanguage}) e 
incluso permite leer de fuentes remotas. Por lo que se concluye que la mejor opción es Config de Lightbend 
\footnote{\url{https://github.com/lightbend/config}}.