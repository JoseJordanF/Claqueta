...
\section{M6: Diseño de la API}

En este hito se pretende llevar a cabo el diseño de la API y su despliegue en local para poder acceder a todos 
...
curva de aprendizaje empinada, que sea modular\cite{modularSoft} para velar que su implementación sea ligera, 
también debe permitir que el formato de las respuestas pueda ser adaptado según las necesidades de la aplicación.
Bajo estos criterios de búsqueda aparecen varias herramientas conocidas, como Spring Boot y la herramienta por la 
que puede ser sustituida Spring Boot, Ktor \cite{ktorVSsboot}. Ambas cumplen con los mínimos requisitos, aunque 
Ktor está guiada para aplicaciones ligeras y escalables, además el uso de Spring Boot es más complejo que el de 
Ktor, y su uso no tiene mucho sentido si no se está creando una aplicación basada en 
Spring\cite{springFrameW}. Por lo que se va a usar Ktor para desplegar la aplicación y establecer las rutas de 
acceso a los recursos.

Para la implementación se ha creado un módulo encargado de inicializar la configuración de las rutas para 
ofrecer respuestas en formato Json, además sé configura el servidor para el despliegue de 
manera sencilla y se inicia el servidor. 
...
esta manera se han diseñado las rutas siguiendo las mejores prácticas \cite{RestPract} y recurriendo a los recursos a 
través de los métodos de la clase mencionada. Además, al principio del código de las rutas se ha incluido el 
complemento de Ktor para permitir que la salida de todas las peticiones sea en formato Json.
